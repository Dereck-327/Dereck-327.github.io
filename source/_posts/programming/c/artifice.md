...
cli_register("help", help_cb, "Show help info");
```

- 字符串常量是用双引号括起来的字符串，比如 "hello"，在 C 语言中称为“字符串字面值”。  
  它的类型是 const char[]，内容在编译时就已经确定。  

- 字符串常量的存储位置  
  - 字符串常量会被编译器存放在程序的**只读数据段（.rodata）**或常量区。  
  - 例如，const char *s = "abc";，"abc" 实际上在程序的只读内存区有一份副本，s 指向这块内存。  

- 程序启动时，所有字符串常量都已经加载到内存。  
  - 只读数据段的内容不会被释放或销毁，直到程序结束。  
    所以，任何时候访问字符串常量的指针都是安全的，不会出现悬挂指针或野指针。
